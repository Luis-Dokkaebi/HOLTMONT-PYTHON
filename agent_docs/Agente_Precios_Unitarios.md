# Software Design Document (SDD): Agente de Precios Unitarios

## 1. Visión General del Componente

Este documento detalla la arquitectura, el diseño y la implementación del **Agente de Precios Unitarios**, una pieza integral de nuestra aplicación impulsada por Inteligencia Artificial.
El propósito principal de este agente es: **Estimar presupuestos y costos unitarios basados en cálculo.**

Se aloja físicamente en el archivo de código fuente `api/paperclip_agents.py`, encapsulado en la función de nodo `precios_node`. A nivel lógico, asume el rol sistémico de **"Analista de Precios Unitarios experto"**.
...
*   **System Message:** Aísla el comportamiento base del modelo ("Eres un Analista de Precios Unitarios experto"). Los LLMs modernos (como LLaMA 3 y Gemini Pro) prestan un peso de atención (Attention Weight) asimétrico superior a los mensajes del sistema en comparación a los del usuario. Esto reduce dramáticamente la tasa de alucinaciones.
...
El ecosistema de la Inteligencia Artificial Generativa no ofrece garantías de disponibilidad 100% (SLA) (debido a rate limits, saturación de GPU, o filtros de seguridad). Por ende, la función `precios_node` asume que el fallo es posible y lo mitiga.
...
def test_agente():
    ...
    resultado = precios_node(estado_falso, llm_mock)
