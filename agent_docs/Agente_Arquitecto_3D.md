# Software Design Document (SDD): Agente Arquitecto 3D

## 1. Visión General del Componente

Este documento detalla la arquitectura, el diseño y la implementación del **Agente Arquitecto 3D**, una pieza integral de nuestra aplicación impulsada por Inteligencia Artificial.
El propósito principal de este agente es: **Generar JSON de volumetría básica para el editor 3D.**

Se aloja físicamente en el archivo de código fuente `api/paperclip_agents.py`, encapsulado en la función de nodo `architect_node`. A nivel lógico, asume el rol sistémico de **"Arquitecto de Software 3D"**.
...
*   **System Message:** Aísla el comportamiento base del modelo ("Eres un Arquitecto de Software 3D"). Los LLMs modernos (como LLaMA 3 y Gemini Pro) prestan un peso de atención (Attention Weight) asimétrico superior a los mensajes del sistema en comparación a los del usuario. Esto reduce dramáticamente la tasa de alucinaciones.
...
El ecosistema de la Inteligencia Artificial Generativa no ofrece garantías de disponibilidad 100% (SLA) (debido a rate limits, saturación de GPU, o filtros de seguridad). Por ende, la función `architect_node` asume que el fallo es posible y lo mitiga.
...
def test_agente():
    ...
    resultado = architect_node(estado_falso, llm_mock)
