# Software Design Document (SDD): Agente de Cálculo y Diseño

## 1. Visión General del Componente

Este documento detalla la arquitectura, el diseño y la implementación del **Agente de Cálculo y Diseño**, una pieza integral de nuestra aplicación impulsada por Inteligencia Artificial.
El propósito principal de este agente es: **Generar requerimientos técnicos y conceptos de obra.**

Se aloja físicamente en el archivo de código fuente `api/paperclip_agents.py`, encapsulado en la función de nodo `calculo_node`. A nivel lógico, asume el rol sistémico de **"Ingeniero Calculista y Arquitecto experto"**.
...
El ecosistema de la Inteligencia Artificial Generativa no ofrece garantías de disponibilidad 100% (SLA) (debido a rate limits, saturación de GPU, o filtros de seguridad). Por ende, la función `calculo_node` asume que el fallo es posible y lo mitiga.
...
def test_agente():
    ...
    resultado = calculo_node(estado_falso, llm_mock)
