# Software Design Document (SDD): Agente Integrador

## 1. Visión General del Componente

Este documento detalla la arquitectura, el diseño y la implementación del **Agente Integrador**, una pieza integral de nuestra aplicación impulsada por Inteligencia Artificial.
El propósito principal de este agente es: **Extraer todos los recursos calculados en un formato JSON estricto.**

Se aloja físicamente en el archivo de código fuente `api/paperclip_agents.py`, encapsulado en la función de nodo `integrador_node`. A nivel lógico, asume el rol sistémico de **"Analista de Datos experto"**.
...
*   **System Message:** Aísla el comportamiento base del modelo ("Eres un Analista de Datos experto"). Los LLMs modernos (como LLaMA 3 y Gemini Pro) prestan un peso de atención (Attention Weight) asimétrico superior a los mensajes del sistema en comparación a los del usuario. Esto reduce dramáticamente la tasa de alucinaciones.
...
El ecosistema de la Inteligencia Artificial Generativa no ofrece garantías de disponibilidad 100% (SLA) (debido a rate limits, saturación de GPU, o filtros de seguridad). Por ende, la función `integrador_node` asume que el fallo es posible y lo mitiga.
...
def test_agente():
    ...
    resultado = integrador_node(estado_falso, llm_mock)
...
### Mecanismos de Sanitización JSON
Observamos la presencia explícita del módulo `json` y bloques de parseo en el código fuente. Esto indica que el agente tiene un alto riesgo de fallo por formatos malformados. Frecuentemente, los LLMs envuelven sus respuestas JSON con delimitadores Markdown (```json ... ```). Para garantizar la robustez, el código debe despojar (strip) cualquier envoltura textual indeseada antes de llamar a `json.loads()`. Esta es una capa defensiva crítica contra las imperfecciones de los modelos Zero-Shot.

### Inyección Estricta mediante Tool Calling
El nodo implementa el paradigma moderno de 'Structured Outputs'. A diferencia de las antiguas técnicas donde dependíamos de que el LLM generase mágicamente un texto fácil de parsear, esta línea invoca a la API utilizando Tool Calling/Function Calling a nivel HTTP. El LLM es obligado por la API del proveedor a devolver llamadas a funciones con los parámetros correspondientes, eliminando el 99% de los fallos de sintaxis en los datos devueltos.
