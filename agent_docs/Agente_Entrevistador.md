# Software Design Document (SDD): Agente Entrevistador

## 1. Visión General del Componente

Este documento detalla la arquitectura, el diseño y la implementación del **Agente Entrevistador**, una pieza integral de nuestra aplicación impulsada por Inteligencia Artificial.
El propósito principal de este agente es: **Generar preguntas para un cliente que solicita cotización.**

Se aloja físicamente en el archivo de código fuente `api/engineering_agent.py`, encapsulado en la función de nodo `interviewer_node`. A nivel lógico, asume el rol sistémico de **"Ingeniero Civil Arquitecto - Generador de Preguntas"**.
...
*   **System Message:** Aísla el comportamiento base del modelo ("Eres un Ingeniero Civil Arquitecto - Generador de Preguntas"). Los LLMs modernos (como LLaMA 3 y Gemini Pro) prestan un peso de atención (Attention Weight) asimétrico superior a los mensajes del sistema en comparación a los del usuario. Esto reduce dramáticamente la tasa de alucinaciones.
...
El ecosistema de la Inteligencia Artificial Generativa no ofrece garantías de disponibilidad 100% (SLA) (debido a rate limits, saturación de GPU, o filtros de seguridad). Por ende, la función `interviewer_node` asume que el fallo es posible y lo mitiga.
...
def test_agente():
    ...
    resultado = interviewer_node(estado_falso, llm_mock)
