# `data/` — Fuentes de datos versionadas

## `denue_cdmx.parquet` y `denue_construccion.parquet`

Directorio Estadístico Nacional de Unidades Económicas (**DENUE**) del INEGI,
entidad 09 (Ciudad de México). Es el insumo del modelo de prospección
geoespacial que vive en el repositorio `ModeloGeo`.
...
`pandas` y `pyarrow` (en `requirements-dev.txt`) hacen falta para *generar* y
para *analizar* estos archivos. No están en `api/requirements.txt` a propósito:
pesan 276 MB instalados junto con numpy y el límite de bundle de una función
serverless de Vercel es de 250 MB. **La API no puede leer Parquet**; cuando el
módulo de prospección exponga endpoints, el subconjunto de construcción tendrá
que servirse en un formato que la biblioteca estándar sepa abrir.
...
información interna de Holtmont.
