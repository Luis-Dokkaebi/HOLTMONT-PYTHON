# Software Design Document (SDD): Agente Evaluador

## 1. Visión General del Componente

Este documento detalla la arquitectura, el diseño y la implementación del **Agente Evaluador**, una pieza integral de nuestra aplicación impulsada por Inteligencia Artificial.
El propósito principal de este agente es: **Analizar la lista de preguntas y rechazar/aprobar.**

Se aloja físicamente en el archivo de código fuente `api/engineering_agent.py`, encapsulado en la función de nodo `evaluator_node`. A nivel lógico, asume el rol sistémico de **"Revisor Senior de Proyectos de Construcción"**.
...
graph TD
    ...
    interviewer_node --> evaluator_node
    evaluator_node -->|is_approved=True| END
    evaluator_node -->|search_queries>0| research_node
    evaluator_node -->|is_approved=False| interviewer_node
    research_node --> interviewer_node
    classDef active fill:#f9f,stroke:#333,stroke-width:4px;
    class evaluator_node active;
```
...
*   **System Message:** Aísla el comportamiento base del modelo ("Eres un Revisor Senior de Proyectos de Construcción"). Los LLMs modernos (como LLaMA 3 y Gemini Pro) prestan un peso de atención (Attention Weight) asimétrico superior a los mensajes del sistema en comparación a los del usuario. Esto reduce dramáticamente la tasa de alucinaciones.
...
El ecosistema de la Inteligencia Artificial Generativa no ofrece garantías de disponibilidad 100% (SLA) (debido a rate limits, saturación de GPU, o filtros de seguridad). Por ende, la función `evaluator_node` asume que el fallo es posible y lo mitiga.
...
def test_agente():
    ...
    resultado = evaluator_node(estado_falso, llm_mock)
